...
<table class="table table-striped table-bordered table-hover">
 ...
 <tr>
	...
	$sql = "SELECT * FROM `cricket` where `Gender`=\"M\" having `contact`";
...
//echo "hello";
		$sno = 1;
				if (mysqli_num_rows($result) > 0) {
					...
					while($row = mysqli_fetch_assoc($result)) {
				   	echo "<tr>";
				    	echo "<td>" . $sno++. "</td>"."<td>" . $row["TEAM"] . "</td>"."<td>" . $row["contact"] . "</td>" .  "<td>". $row["Roll_no"] . "</td>"."<td>" . $row["Capt_name"] . "</td>" ; 
				    	echo "</tr>";
				  //  echo "hello11";
 ...
 <tr>
	...
	$sql = "SELECT * FROM `cricket` where `Gender`=\"F\" having `contact`";
