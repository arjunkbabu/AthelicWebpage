...
*/
	...
	$sql = "SELECT `result_hammer_m`.`chest` , `result_hammer_m`.`score` , `roll` , `name` , `batch`
    FROM `result_hammer_m` , `STUDENT` WHERE `result_hammer_m.`chest` = `STUDENT`.`CHEST` AND 
    `result_hammer_m`.`score` != 0
    ORDER BY `result_hammer_m.`score` DESC
    LIMIT 0 , 5";
             ...
             <!--header section-->
               <div class="header" class="row">
                ...
                <!-- Registration form -->
                 ...
                 <div id="legend">
                <h3 class="text-center" class="subhead"> HAMMER THROW (FEMALE) </h3>
                </div>
                <!--Name-->
...
<!--		<form method = "post" action = "add_hscore_100M_handler.php"> -->
			...
			if (mysqli_num_rows($result) != 0) {
				...
				while($row = mysqli_fetch_assoc($result)) {
					echo "<tr>";
					echo "<td>".$count++."</td>"." "."<td>".$row['chest']."</td>"." "."<td>".$row['name']."</td>"." "."<td>".$row['roll']."</td>"." "."<td>".$row['score']."</td>"." "."<td>".$row['score']."</td>";
					echo "</tr>";
